...
double detectColoredNeedle(const cv::Mat &frame, const GaugeROI &gauge) {
    ...
    for (size_t i = 0; i < contours.size(); i++) {
        ...
        if (cv::norm(centroid - gauge.center) > gauge.radius * 0.6) continue;
        double maxDist = 0;
        for (const auto &pt : contours[i])
            maxDist = std::max(maxDist, cv::norm(pt - gauge.center));
...
double detectNeedleRadial(const cv::Mat &frame, const GaugeROI &gauge) {
    ...
    for (int i = 0; i < numAngles; i++) {
        ...
        double reach = totalScan > 0 ? (double)longestRun / gauge.radius : 0;
...
std::vector<TickMark> scanRingAtRadius(const cv::Mat &frame, const GaugeROI &gauge,
                                        double scanRadius, int numAngles) {
    cv::Mat gray;
    cv::cvtColor(frame, gray, cv::COLOR_BGR2GRAY);
...
void drawOverlay(cv::Mat &frame, const GaugeROI &gauge,
    ...
    if (gauge.radius > 0) {
        ...
        if (scale.valid) {
            ...
            cv::putText(frame, std::to_string((int)scale.minValue), startPt + cv::Point(10, 0),
            ...
            cv::putText(frame, std::to_string((int)scale.maxValue), endPt + cv::Point(10, 0),
